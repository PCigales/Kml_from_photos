...
path_kml = (dir_dest + name + '.kml') if is_kml else (name + ' - kmz.kml')
path_kmz = '' if is_kml else dir_dest + name + '.kmz'
...
base_thumbs = ('file:///' + pref_thumbs.replace('\\', '/')) if is_kml else ('thumbs/%s - ' % name)
...
kml.write('<description><![CDATA[Images du répertoire %s]]></description>\r\n' % dir_scan[:-1])
...
kml.write('<text>\r\n')
kml.write('<![CDATA[\r\n')
kml.write('<style type="text/css">\r\n')
kml.write('.ori0 {max-width:300px;max-height:200px;}\r\n')
kml.write('.ori1 {max-width:200px;max-height:300px;-webkit-transform:rotate(90deg) translate(0px,-100%);-webkit-transform-origin:top left;}\r\n')
kml.write('.ori2 {max-width:200px;max-height:300px;-webkit-transform:rotate(270deg) translate(-100%,0px);-webkit-transform-origin:top left;}\r\n')
kml.write('.ori3 {max-width:300px;max-height:200px;-webkit-transform:rotate(180deg) translate(-100%,-100%);-webkit-transform-origin:top left;}\r\n')
kml.write('</style>\r\n')
kml.write('<div style="float:left;width:200px;height:1.2em;overflow:hidden;"><b>%s</b></div>\r\n' % name)
kml.write('<div style="float:right;padding-top:0.2em;font-size:0.8em;">$[date]</div><br/><hr/>\r\n')
kml.write('<b>Lat: </b><code>$[lat]</code>&nbsp;&nbsp;&nbsp;<b>Lon: </b><code>$[lon]</code><br/><br/>\r\n')
kml.write('<div style="width:308px;height:200px;"><img src="$[url]" class="ori$[ori]"%s/></div><br/>\r\n' % ('' if is_kml else ' onerror="if (this.getAttribute(&quot;src&quot;).substring(0,1)&excl;=&quot;.&quot;) {this.src=(window.location.href.split(&quot;.&quot;).pop().slice(0,3).toLowerCase()==&quot;kmz&quot;?&quot;../&quot;:&quot;./&quot;)+this.src.split(&quot;/&quot;).pop();} else {this.src=&quot;$[urlt]&quot;;this.className=&quot;ori0&quot;;this.onerror=null;}"'))
kml.write('<div style="width:300px;"><b>Nom: </b><a href="" id="lien">$[name]</a></div><hr/>\r\n')
kml.write('<script>\r\n')
kml.write('var p="$[ori]"+encodeURIComponent("$[url]").replace(/%/g,"*");\r\n')
kml.write('document.getElementById("lien").href=%sviewer.htm#"+p;\r\n' % (('"file:///' + dir_dest.replace('\\', '/')) if is_kml else '(window.location.href.split(".").pop().slice(0,3).toLowerCase()=="kmz"?"..":".")+"/'))
kml.write('</script>\r\n')
kml.write('<br/>$[geDirections]]]>\r\n')
kml.write('</text>\r\n')
kml.write('</BalloonStyle>\r\n')
kml.write('</Style>\r\n')
kml.write('<Style>\r\n')
...
for p in lst:
    ...
    if not exist_t:
      ...
      try:
        ...
        if wic.IWICImagingFactory_CreateBitmapScaler_Proxy(pFactory, ctypes.byref(pIBitmapScaler)):
          raise
        w, h = (150, round(150 / uiWidth.value * uiHeight.value)) if uiWidth.value >= uiHeight.value else (round(150 / uiHeight.value * uiWidth.value), 150)
  ...
  kml.write('<Data name="url"><value>file:///%s</value></Data>\r\n' % p.replace('\\', '/'))
  kml.write('<Data name="ori"><value>%d</value></Data>\r\n' % {3: 3, 4: 3, 5: 1, 6: 1, 7: 2, 8: 2}.get(r, 0))
  if not is_kml:
    kml.write('<Data name="urlt"><value>%s%s.jpg</value></Data>\r\n' % (base_thumbs, name_p))
  kml.write('</ExtendedData>\r\n')
