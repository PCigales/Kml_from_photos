...
- option 2 = use Windows Imaging Component directly for thumbnail generation and metadata extraction (a bit faster):
  ...
  * to improve the quality (hardly noticeable, but about one and a half as slow) of the thumbnails, it is possible to change the interpolation algorithm from "Fant" to "HighQualityCubic" (requires Windows >= 10) by replacing, in both Python scripts, "wic.IWICBitmapScaler_Initialize_Proxy(pIBitmapScaler, pIBitmap1, ctypes.wintypes.UINT(w), ctypes.wintypes.UINT(h), ctypes.wintypes.UINT(3))" by "wic.IWICBitmapScaler_Initialize_Proxy(pIBitmapScaler, pIBitmapFrame, ctypes.wintypes.UINT(w), ctypes.wintypes.UINT(h), ctypes.wintypes.UINT(4))"
  * to increase the speed (almost twice as fast but with a barely noticeable quality degradation of the thumbnails in the form of an offset of the pixels when greatly magnified), it is possible to directly apply the "Fant" interpolation algorithm to the source picture allowing pre-shrinking at decode time, by replacing, in both Python scripts, "wic.IWICBitmapScaler_Initialize_Proxy(pIBitmapScaler, pIBitmap1, ctypes.wintypes.UINT(w), ctypes.wintypes.UINT(h), ctypes.wintypes.UINT(3))" by "wic.IWICBitmapScaler_Initialize_Proxy(pIBitmapScaler, pIBitmapFrame, ctypes.wintypes.UINT(w), ctypes.wintypes.UINT(h), ctypes.wintypes.UINT(3))"
